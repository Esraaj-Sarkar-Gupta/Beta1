...
def run(transm):
    transmissibility = transm
    log_filename = project_dir + '/Logs/tr=' + str(transm) + '.txt'
    save_dir = project_dir + '/Figures/tr=' + str(transm)
    ...
    plt.figure(figsize=(10,10))
    plt.title(f"Evolution of System - Transmissibility = {transm}")
    ...
    plt.grid(True)
    plt.savefig(f"{project_dir}/transm={transm}.png")
...
for value in range(1, 5 + 1):
    print(f"> Running decay = {value / 1000}")
    run(value / 10)
