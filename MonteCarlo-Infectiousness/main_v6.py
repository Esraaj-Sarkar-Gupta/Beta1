...
def run(infec):
    ...
    log_filename = project_dir + '/Logs/ifc=' + str(infec) + '.txt'
    save_dir = project_dir + '/Figures/ifc=' + str(infec)
    ...
    plt.figure(figsize=(10,10))
    plt.title(f"Evolution of System - Infectiousness = {infec}")
    ...
    plt.grid(True)
    plt.savefig(f"{project_dir}/infec={infec}.png")
...
for value in [1, 1.1, 1.2, 1.3, 1.4, 1.5]:
    print(f"> Running decay = {value}")
    run(value)
